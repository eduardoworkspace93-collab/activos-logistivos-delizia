...
async function startServer() {
  ...
  const checkAuth = (allowedRoles?: ('Administrador' | 'Supervisor' | 'Operador' | 'OPC' | 'OPP' | 'OPA')[]) => {
  ...
  app.post('/api/auth/login', (req, res) => {
    ...
    if (isOperatorRole && (!user.area || user.area === 'general')) {
      return res.status(403).json({ error: 'Acceso denegado: El área Planta El Alto es de acceso exclusivo para personal jerárquico.' });
    }
  ...
  app.post('/api/items', checkAuth(['Administrador', 'Supervisor', 'Operador', 'OPC', 'OPP', 'OPA']), (req: any, res) => {
    try {
  ...
  app.post('/api/locations', checkAuth(['Administrador', 'Supervisor', 'Operador', 'OPC', 'OPP', 'OPA']), (req: any, res) => {
    try {
  ...
  app.post('/api/responsibles', checkAuth(['Administrador', 'Supervisor', 'Operador', 'OPC', 'OPP', 'OPA']), (req: any, res) => {
    try {
  ...
  app.post('/api/drivers-routes', checkAuth(['Administrador', 'Supervisor', 'Operador', 'OPC', 'OPP', 'OPA']), (req: any, res) => {
    try {
  ...
  // 3e. Operational Areas Management (Áreas / Estados de Stock)
  app.get('/api/operational-areas', checkAuth(), (req, res) => {
    res.json(db.getOperationalAreas());
  });

  app.post('/api/operational-areas', checkAuth(['Administrador', 'Supervisor', 'Operador', 'OPC', 'OPP', 'OPA']), (req: any, res) => {
    try {
      const area = db.saveOperationalArea(req.body, req.user.username, getClientIp(req));
      res.json(area);
    } catch (e: any) {
      res.status(400).json({ error: e.message });
    }
  ...
  app.delete('/api/operational-areas/:id', checkAuth(['Administrador']), (req: any, res) => {
    try {
      const success = db.removeOperationalArea(req.params.id, req.user.username, getClientIp(req));
      if (success) {
