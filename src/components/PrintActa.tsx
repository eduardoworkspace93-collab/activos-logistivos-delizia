          ...
          <div className="grid grid-cols-2 gap-y-3 gap-x-4 border-b border-slate-200 pb-4 mb-4">
            ...
            <div>
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">
                {movement.fromStatus ? 'Flujo de Traspaso Interno' : 'Estado Físico / Destino'}
              </span>
              <span className={`inline-flex items-center gap-1.5 font-bold ${
                movement.crateStatus === 'Dañado' || movement.crateStatus === 'Reparación' ? 'text-red-600' : 'text-emerald-600'
              }`}>
                {movement.fromStatus ? (
                  <span className="flex items-center gap-1 text-slate-700 font-sans text-[10px] mt-0.5">
                    <span className="bg-amber-50 text-amber-800 text-[9px] px-1.5 py-0.5 rounded border border-amber-200 uppercase">
                      {movement.fromStatus === 'Planta' || movement.fromStatus === 'Planta_Disponibles' ? 'ACTIVOS LOGÍSTICOS' :
                       movement.fromStatus === 'Produccion' ? 'PRODUCCIÓN' :
                       movement.fromStatus === 'Planta_Almacen' ? 'ALMACÉN PRODUCTO TERMINADO' : movement.fromStatus}
                    </span>
                    <span className="text-slate-400">➔</span>
                    <span className="bg-sky-50 text-[#003366] text-[9px] px-1.5 py-0.5 rounded border border-sky-200 uppercase font-extrabold">
                      {movement.crateStatus === 'Planta' || movement.crateStatus === 'Planta_Disponibles' ? 'ACTIVOS LOGÍSTICOS' :
                       ...
                       movement.crateStatus === 'Planta_Almacen' ? 'ALMACÉN PRODUCTO TERMINADO' : movement.crateStatus}
                    </span>
                  </span>
                ) : (
