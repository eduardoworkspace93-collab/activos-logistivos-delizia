...
import { Search, ArrowUpDown, FileSpreadsheet, AlertTriangle, Layers } from 'lucide-react';
...
export default function Stock({ stocks, userRole, currentUser }: StockProps) {
  ...
  // Detect restriction by role or area (OPC, OPP, OPA, or custom Operator)
  const isRestricted = (
    currentUser?.role === 'OPC' || 
    currentUser?.role === 'OPP' || 
    currentUser?.role === 'OPA' || 
    (currentUser?.role === 'Operador' && currentUser?.area && currentUser?.area !== 'general')
  );

  const userArea = currentUser?.area || (
    currentUser?.role === 'OPC' ? 'activos' :
    currentUser?.role === 'OPP' ? 'produccion' :
    currentUser?.role === 'OPA' ? 'almacen' : 'general'
  );

  const getAreaMetadata = (area: string) => {
    switch (area) {
      case 'activos':
        return {
      ...
      case 'produccion':
        return {
          label: 'Stock Producción',
          field: 'produccion' as const,
          description: 'Área de Producción (Llenado y Envasado)'
        };
      case 'almacen':
        return {
  ...
  const areaMeta = getAreaMetadata(userArea);
  ...
  const handleExportExcel = () => {
    ...
    if (isRestricted && areaMeta) {
      formattedData = stocks.map(s => ({
        code: s.code,
        name: s.name,
        areaStock: s.breakdown[areaMeta.field] || 0,
        status: s.status
      }));
      ...
      headers = [
        ...
        { key: 'areaStock', label: `STOCK ${areaMeta.label.toUpperCase()}` },
    ...
    exportToCSV(formattedData, `Control_Inventario_Activos_Delizia_${userArea}`, headers);
  };
  ...
  const filteredStocks = stocks
    .filter(s => {
      ...
      if (statusFilter === 'Bajo Stock') {
        const valueToCheck = isRestricted && areaMeta ? (s.breakdown[areaMeta.field] || 0) : s.stockActual;
        return valueToCheck < 50 && s.status === 'Activo';
      }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 pt-1">
          ...
          <div>
            ...
            >
              ...
              <option value="Bajo Stock">{isRestricted && areaMeta ? `Bajo Stock en Área (<50 u)` : 'Bajo Stock General (<50 u)'}</option>
            </select>
          </div>
      ...
      {isRestricted && areaMeta && (
        <div className="bg-[#003366]/5 border border-[#003366]/15 p-3 rounded-lg text-xs flex gap-2.5 items-center">
          <span className="text-base">🔒</span>
          <div>
            <p className="font-bold text-[#003366]">Vista de Stock Restringida</p>
            <p className="text-slate-500 text-[10px] mt-0.5 leading-relaxed">
              Su cuenta tiene asignado el rol/área de <b>{areaMeta.description}</b>. 
              Por políticas de control interno, solo tiene visibilidad del inventario físico en su sector correspondiente.
            </p>
          </div>
        </div>
      )}
            ...
            <thead>
              ...
              {isRestricted && areaMeta ? (
                <tr className="bg-slate-100 border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                  ...
                  <th className="p-2 text-center border-r border-slate-200 bg-sky-50/20 text-[#003366]">Inventario Autorizado de su Área</th>
                  <th className="p-2 text-center">Estado del Canastillo</th>
                </tr>
              ) : (
                <tr className="bg-slate-100 border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                  <th colSpan={2} className="p-2 px-4 border-r border-slate-200">Datos del Activo</th>
              ...
              {isRestricted && areaMeta ? (
                <tr className="bg-slate-50 border-b border-slate-200 text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                  ...
                  <th className="p-2.5 text-center font-extrabold text-[#003366] bg-[#003366]/5 border-r border-slate-200">
                    {areaMeta.label.toUpperCase()}
                  </th>
            ...
            <tbody className="divide-y divide-slate-150 text-[11px]">
              {filteredStocks.length === 0 ? (
                ...
                filteredStocks.map(s => {
                  ...
                  if (isRestricted && areaMeta) {
                    const areaStockValue = s.breakdown[areaMeta.field] || 0;
                    const isAreaLow = areaStockValue < 50;
                    return (
                      <tr key={s.id} className="hover:bg-slate-50/40">
                        ...
                        <td className="p-2.5 text-center font-mono font-extrabold text-sm text-[#003366] bg-[#003366]/5 border-r border-slate-200">
                          <span className={isAreaLow ? 'text-red-500 font-extrabold' : ''}>
                            {areaStockValue} u.
                          </span>
                        </td>
