...
export default function Movimientos({ 
  ...
  const areaToStatusMap: Record<string, string> = {
    'activos': 'Planta_Disponibles',
    'produccion': 'Produccion',
    'almacen': 'Planta_Almacen'
  };
  ...
  // Filter history based on operator area restriction
  const filteredHistoryMovements = React.useMemo(() => {
    return movements.filter(m => {
      if (isRestrictedOperator && restrictedStatus) {
        return m.crateStatus === restrictedStatus || m.fromStatus === restrictedStatus;
      }
      return true;
    });
  }, [movements, isRestrictedOperator, restrictedStatus]);
  ...
  const handleAddDraftItem = () => {
    ...
    if (fromStatus) {
      ...
      if (selectedStockInfo && totalRequestedQty > selectedStockInfo.stockActual) {
        setFormError(`Stock insuficiente para "${selectedItem.name}". Stock disponible total: ${selectedStockInfo.stockActual}. Ya has agregado ${alreadyAddedQty} a la lista, e intentas agregar ${quantity} más.`);
        return;
      }
    }
          ...
          <div className="lg:col-span-2 bg-white p-4 rounded-lg border border-slate-200 shadow-sm space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1">N° de Documento (Opcional - Autogenerado)</label>
                  <input
                    type="text"
                    value={orderNumber}
                    onChange={e => setOrderNumber(e.target.value.toUpperCase())}
                    placeholder="Ej. Sin doc. o autogenerado"
                    ...
                    <table className="w-full text-left border-collapse text-[11px]">
                      <thead>
                        <tr className="bg-slate-50 text-[9px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                          ...
                          <th className="p-2 px-3">Flujo (Origen ➔ Destino)</th>
                ...
                {filteredHistoryMovements.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="p-8 text-center text-slate-400">
                      <AlertCircle className="w-6 h-6 mx-auto opacity-30 mb-1.5" />
                      <span>No hay movimientos registrados para su área</span>
                    </td>
                  </tr>
                ) : (
                  [...filteredHistoryMovements].reverse().slice(0, 50).map(m => (
