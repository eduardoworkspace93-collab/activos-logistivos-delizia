...
export default function Usuarios({ users, currentUser, onSaveUser }: UsuariosProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isOperatorRole = ['Operador', 'OPC', 'OPP', 'OPA'].includes(editUser.role);
    if (isOperatorRole && (!editUser.area || editUser.area === 'general')) {
      setFormError('Los operadores no pueden tener asignada el área "Planta El Alto". Debe asignarle una de las tres áreas de operación específicas.');
      return;
    }
      ...
      {isEditing && isAdmin ? (
        ...
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm max-w-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    let newArea = editUser.area;
                    if (newRole === 'OPC') newArea = 'activos';
                    else if (newRole === 'OPP') newArea = 'produccion';
                    else if (newRole === 'OPA') newArea = 'almacen';
                    else if (['Operador', 'OPC', 'OPP', 'OPA'].includes(newRole) && (!newArea || newArea === 'general')) newArea = 'activos';
                    setEditUser({ ...editUser, role: newRole, area: newArea });
                  }}
                ...
                >
                  ...
                  <option value="Operador" className="text-slate-900 bg-white">Operador Estándar</option>
                  <option value="OPC" className="text-slate-900 bg-white">Operador Canastillos (OPC)</option>
                  <option value="OPP" className="text-slate-900 bg-white">Operador Producción (OPP)</option>
                  <option value="OPA" className="text-slate-900 bg-white">Operador Almacén (OPA)</option>
                </select>
              </div>
            ...
            <div>
              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Área de Trabajo Asignada</label>
              <select
                value={editUser.area || 'general'}
                onChange={e => setEditUser({ ...editUser, area: e.target.value as any })}
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs focus:bg-white focus:outline-none text-slate-800"
              >
                {!['Operador', 'OPC', 'OPP', 'OPA'].includes(editUser.role) && (
                  <option value="general" className="text-slate-900 bg-white">Planta El Alto (Acceso Total)</option>
                )}
                <option value="activos" className="text-slate-900 bg-white">Activos Logísticos (Recepción/Vacíos)</option>
                <option value="produccion" className="text-slate-900 bg-white">Producción (Envasado/Llenado)</option>
                <option value="almacen" className="text-slate-900 bg-white">Almacén de PT (Cámaras/Despacho)</option>
              </select>
              <p className="text-[9px] text-slate-400 mt-1 leading-normal">
                Determina a qué área física o sucursal pertenecerá este operador, limitando su visión de stock y habilitando pre-llenados automatizados de movimientos.
              </p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  ...
                  <th className="p-2.5 px-4">Área de Trabajo</th>
              ...
              <tbody className="divide-y divide-slate-150 text-[11px]">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-slate-50/40">
                    ...
                    <td className="p-2.5 px-4 font-semibold text-slate-700">
                      {u.area === 'activos' && '🏭 Activos Logísticos'}
                      {u.area === 'produccion' && '📦 Producción'}
                      {u.area === 'almacen' && '❄️ Almacén de PT'}
                      {(!u.area || u.area === 'general') && '🏢 Planta Central (Total)'}
                    </td>
                    <td className="p-2.5 px-4">
                    ...
                    <td className="p-2.5 px-4 text-slate-500">
                      ...
                      {u.role === 'OPC' && 'Operador Canastillos: Registro de movimientos y stock de Vacíos'}
                      {u.role === 'OPP' && 'Operador Producción: Registro de movimientos y stock de Producción'}
                      {u.role === 'OPA' && 'Operador Almacén: Registro de movimientos y stock de Almacén PT'}
                    </td>
                    {isAdmin && (
