...
export class LocalDatabase {
  ...
  async initFirestore() {
    ...
    try {
      const collections = ['users', 'items', 'movements', 'kardex', 'audit', 'config', 'locations', 'responsibles', 'driversRoutes'];
  ...
  registerMovement(moveData: Omit<Movement, 'id' | 'movementNumber' | 'date' | 'time'>, operatorUsername: string, ip: string): Movement {
    const documentNumber = (moveData.documentNumber || moveData.orderNumber || '').trim();
    if (!documentNumber) {
      throw new Error('El número de documento es obligatorio y no puede estar vacío.');
    }
    ...
    if (moveData.type === 'salida' && currentStock < quantity) {
  ...
  registerMovementBatch(
    ...
    const documentNumber = (batchData.documentNumber || '').trim();
    if (!documentNumber) {
      throw new Error('El número de documento es obligatorio y no puede estar vacío.');
    }
    ...
    for (let index = 0; index < batchData.items.length; index++) {
      ...
      if (batchData.type === 'salida' && currentStock < quantity) {
